...
* `rdiff` (the difference between x and y radius) and
...
Instead of list with parameter values as in previous examples, 
we can use this data frame with explicit parameter sets 
to define `param_values`:
...
```{r title = "Show fur patterns", furShowPatches}
library(ggplot2)
